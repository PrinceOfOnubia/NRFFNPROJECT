...
function isVideo(src: string) {
  return /\.(mp4|webm|ogg|mov)$/i.test(src) || src.startsWith("video/");
}
    ...
    <div className="npl-gallery">
      <div className="npl-gallery__stage">
        {currentIsVideo ? (
          ...
          <img src={current} alt={`${name} — view ${i + 1}`} />
        )}
      ...
      <div className="npl-gallery__thumbs">
        {slides.map((src, n) => (
          <button key={src + n} className={`npl-gallery__thumb${n === i ? " active" : ""}`} onClick={() => setI(n)}>
            {isVideo(src) ? <video src={src} muted playsInline preload="metadata" /> : <img src={src} alt={`${name} thumbnail ${n + 1}`} />}
          </button>
        ))}
      </div>
      ...
      {zoom && (
        <div className="npl-gallery__lightbox" onClick={() => setZoom(false)} role="presentation">
          ...
          {currentIsVideo ? (
            <video src={current} controls autoPlay playsInline onClick={(e) => e.stopPropagation()} />
          ) : (
            <img src={current} alt={`${name} — view ${i + 1}`} onClick={(e) => e.stopPropagation()} />
          )}
